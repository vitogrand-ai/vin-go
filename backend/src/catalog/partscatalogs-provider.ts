...
const NODE_HINTS: [RegExp, string[]][] = [
  ...
  // Колодки живут в схеме тормозного механизма рядом с диском: у Subaru лист
  // «Колодки тормозные» ведёт только в задний тормоз, передние — в «Диске».
  [/колодк/, ['диск, барабан тормозной', 'дисковый тормозной механизм']],
  // Стойка стабилизатора у Subaru — в листе «Сайлентблоки, втулки стабилизатора».
...
function pickLeaves(leaves: TreeLeaf[], query: string): TreeLeaf[] {
  ...
  return leaves
    .map((leaf, order) => {
      const path = leaf.path.toLowerCase().replaceAll('ё', 'е')
      // Узел по подсказке уступает только узлу, названному самой деталью, но не
      // листу с одним общим словом: живьём 23.09.2026 «Крышка багажника» и
      // «Крышка ГБЦ» (0.6 по слову «крышка») вытесняли «Бачок расширительный»
      // из трёх раскрываемых листьев, и крышка бачка не находилась.
      const hinted = hints.some((hint) => path.includes(hint)) ? HINTED_LEAF_SCORE : 0
