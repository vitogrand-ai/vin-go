...
/**
 * Номер выноски на схеме: короткое целое число и ничего больше. Пять знаков —
 * потолок нумерации: у Ford позиции пятизначные (АКБ Mondeo — «10655», живьём
 * сентябрь 2026, бот искал такой номер как название). Длинные числа (артикул)
 * сюда не попадают, и число берётся за позицию, только пока показана схема.
 */
