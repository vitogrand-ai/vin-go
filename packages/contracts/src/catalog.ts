...
/**
 * Расходники планового ТО — кнопка «Детали ТО»: мастер получает их одним
 * нажатием вместо пяти вопросов. Каждый пункт ищется обычным поиском по
 * `query`, поэтому номера приходят из каталога тем же путём, что и всегда.
 *
 * Состав сверен замером 25.09.2026 на 10 машинах пилота: масляный,
 * воздушный и салонный фильтры и свечи находятся первой строкой на 9-10
 * машинах из 10; топливный — на 6 (у многих бензиновых он встроен в модуль
 * насоса в баке, и честный ответ — «не найдено»). У дизеля свечей зажигания
 * нет — пусто там тоже честно.
 */
export const MAINTENANCE_ITEMS = [
  { label: 'Масляный фильтр', query: 'масляный фильтр' },
  { label: 'Воздушный фильтр', query: 'воздушный фильтр' },
  { label: 'Салонный фильтр', query: 'салонный фильтр' },
  { label: 'Топливный фильтр', query: 'топливный фильтр' },
  { label: 'Свечи зажигания', query: 'свечи зажигания' },
] as const
